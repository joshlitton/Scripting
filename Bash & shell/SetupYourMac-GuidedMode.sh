...
scriptLog="${4:-"/var/log/setupYourMac.log"}"                        # Parameter 4: Script Log Location [ /var/log/org.churchofjesuschrist.log ] (i.e., Your organization's default location for client-side logs)
...
departmentListRaw="Managed Services,Operations,Customer Success,Professional Services,Director"

# A sorted, unique, JSON-compatible list of departments
departmentList=$( echo "${departmentListRaw}" | tr ',' '\n' | sort -f | uniq | sed -e 's/^/\"/' -e 's/$/\",/' -e '$ s/.$//' )

# Branding overrides
brandingBanner="https://static.wixstatic.com/media/3a8935_387ce127796c47daa81ebf5b9695088d~mv2.jpg/v1/fill/w_1453,h_1080,al_c,q_85,enc_auto/3a8935_387ce127796c47daa81ebf5b9695088d~mv2.jpg"
brandingBannerDisplayText="true"
brandingIconLight="https://citwapro.jamfcloud.com/api/v1/icon/download/43219"
brandingIconDark="https://citwapro.jamfcloud.com/api/v1/icon/download/43219"
...
if [ "$promptForAssetTag" == "true" ]; then
    assetTagJSON='{   "title" : "Asset Tag",
        "required" : true,
        "prompt" : "Please enter the seven-digit Asset Tag starting with CAT",
        "regex" : "^CAT[0-9]{4,4}$",
        "regexerror" : "Please enter CAT followed by 4 digits for the Asset Tag. (eg. CAT1234)"
    },'
fi
...
setupYourMacPolicyArrayIconPrefixUrl="https://citwapro.jamfcloud.com/api/v1/icon/download/"
                ...
                "steps": [
                	{
                        "listitem": "Utilities",
                        "icon": "43222",
                        "progresstext": "Installing utilities to facilitate deployment and setting up profile",
                        "trigger_list": [
                        	{
                            ...
                            {
                                "trigger": "install-desktop",
                                "validation": "None"
                            },
                            {
                                "trigger": "install-dock",
                                "validation": "None"
                            }
                        ]
                    },
                    ...
                    {
                        "listitem": "Jamf Protect",
                        "icon": "43193",
                        "progresstext": "Jamf Protect is a purpose-built endpoint security and mobile threat defense (MTD) for Mac and mobile devices.",
                        "trigger_list": [
                            {
                                "trigger": "install-jamfprotect",
                                "validation": "/Applications/JamfProtect.app/Contents/Info.plist"
                            }
                        ]
                    },
                    {
                        ...
                        "icon": "37776",
                        "progresstext": "The king of web browsers, unparalleled feature set, excellent render speed and not a glutton for RAM.",
                        "trigger_list": [
                            {
                                "trigger": "install-edge",
                    ...
                    {
                        "listitem": "Profile Configuration",
                        "icon": "43206",
                        "progresstext": "Finalizing Configuration …",
                        "trigger_list": [
                            {
                    ...
                    {
                        "listitem": "Support Agent",
                        "icon": "43229",
                        "progresstext": "SupportApp is a tool bar agent that provides links and system information with a single click",
                        "trigger_list": [
                            {
...
function finalise(){
    ...
    if [[ "${jamfProPolicyTriggerFailure}" == "failed" ]]; then
        ...
        if [[ "${brandingBannerDisplayText}" == "true" ]] ; then
        	echo "Commented out to remove title"
